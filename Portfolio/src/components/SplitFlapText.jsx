...
/**
 * Airport/train-board style split-flap readout. Two modes:
 *  - `words`: cycles through a list on its own timer (`cycleDelay`), looping.
 *  - `text`: a controlled single value (e.g. a live percentage) — the first
 *    mount snaps in place, every value after that flips in immediately, so
 *    it can be driven directly from external state instead of a timer.
 */
const SplitFlapText = ({
  words = DEFAULT_WORDS,
  text,
  ...
  const isControlled = typeof text === 'string';
  const sourceWords = Array.isArray(words) && words.length > 0 ? words : DEFAULT_WORDS;
  const phrasesKey = isControlled ? text : sourceWords.map(word => String(word ?? '')).join('');
  ...
  useEffect(() => {
    ...
    const snapTo = phrase => {
      const snapped = createTiles(phrase);
      tilesRef.current = snapped;
      setTiles(snapped);
    };
    ...
    const animateTo = targetPhrase => {
      ...
      const fromPhrase = normalizePhrase(tilesRef.current.map(tile => tile.current).join(''), width);
    ...
    if (typeof window === 'undefined') return clearAnimation;
